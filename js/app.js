...
function renderSlotsAndLimits(){
  const out = spellSlotsForState(state);
...
function renderSpells(){
  ...
  // Warlock-only: single spell list.
...
function initStaticBindings(){
  ...
  // Warlock-only: no secondary spell ability mod.
...
function renderSelectValues(){
  ...
  if ($("primarySpellMod")) $("primarySpellMod").value = String(state.primary?.spellMod || 0);

  $("notes").value = state.notes || "";
  $("spellNotes").value = state.spells?.notes || "";

  // Warlock-only: no multiclass UI.

  if ($("primaryClass")) $("primaryClass").value = (state.primary?.className || "").trim();
  if ($("primaryLevel")){
    $("primaryLevel").value = String(state.primary?.classLevel ?? 0);
    $("primaryLevel").disabled = !!state.build?.locked;
  }

  // Warlock-only: no secondary class UI.
...
function hasFeatId(st, featId){
  const id = String(featId||"").trim();
  if (!id) return false;
  if (Array.isArray(st?.feats) && st.feats.some(f => f?.id === id)) return true;
  if (Array.isArray(st?.picks) && st.picks.some(p => p?.type === "feat" && p?.featId === id)) return true;
  return false;
}

function toughBonusOnLevelUp({ stateBefore, gainingToughNow, newTotalLevel }){
  const hadTough = hasFeatId(stateBefore, "feat_tough");
  if (gainingToughNow && !hadTough){
    // PHB 2014: when you gain Tough, increase max HP by twice your level.
    return 2 * clampInt(newTotalLevel, 1, 20);
  }
  if (hadTough) return 2; // +2 HP each level after gaining it.
  return 0;
}
...
function buildSpellLearnPlan({ className, subclassName, fromLevel, toLevel }){
  ...
  const plan = {
    ...
    spellListLabel: "Known",
  };

  if (cn !== "Warlock") return plan;

  const kFrom = spellsKnownLimit("Warlock", from) ?? 0;
  const kTo = spellsKnownLimit("Warlock", to) ?? 0;
  const cFrom = cantripsKnownLimit("Warlock", from) ?? 0;
  const cTo = cantripsKnownLimit("Warlock", to) ?? 0;

  plan.spellsToChoose = Math.max(0, Number(kTo) - Number(kFrom));
  plan.cantripsToChoose = Math.max(0, Number(cTo) - Number(cFrom));
...
function renderLevelUpWizardStep(){
  ...
  if (stepId === "choose"){
    ...
    LEVELUP_WIZ.which = "primary";

    body.innerHTML = `
      <div class="small muted">Current total level: <strong>${t}</strong></div>
      <div class="mt">
        <div class="pill">Class: <strong>${escapeHtml(pName)}</strong> (level ${pLv})</div>
      </div>
      <div class="small muted mt">Level cap is 20.</div>
  ...
  if (stepId === "subclass"){
    const cls = (LEVELUP_WIZ.className||"").trim();
    const subs = (DATA.subclasses?.classes||[]).find(c => c.class === cls)?.subclasses || [];
    const cur = (LEVELUP_WIZ.subclassName||"").trim();
    body.innerHTML = `
      <div class="small muted">Choose a subclass for <strong>${escapeHtml(cls)}</strong>.</div>
      <label class="field mt">
        <span>Subclass</span>
        <select id="wizSubclassSel">
          <option value="">(choose)</option>
          ${subs.map(s => `<option value="${escapeAttr(s.name)}">${escapeHtml(s.name)}</option>`).join("")}
        </select>
      </label>
      <div class="small muted mt">This records your level 1 subclass choice (if your class selects a subclass at level 1).</div>
      <div id="wizErr" class="wiz-err small mt"></div>
    `;
    const sel = body.querySelector("#wizSubclassSel");
    if (sel){
      sel.value = cur;
      sel.addEventListener("change", () => { LEVELUP_WIZ.subclassName = sel.value; });
    }
    return;
  }
  ...
  if (stepId === "choices"){
    const cls = (LEVELUP_WIZ.className||"").trim();
    const lv = clampInt(LEVELUP_WIZ.toLevel, 1, 20);
    const which = LEVELUP_WIZ.which;
    const cd = DATA.classFeatures?.classes?.[cls] || null;
    const entry = cd?.levels?.[String(lv)] || null;
    const choices = Array.isArray(entry?.choices) ? entry.choices : [];

    if (!choices.length){
      body.innerHTML = `<div class="muted">No class feature choices at ${escapeHtml(cls)} level ${lv}.</div>`;
      return;
    }

    body.innerHTML = `
      <div class="small muted">Make class feature choices for <strong>${escapeHtml(cls)}</strong> level ${lv}.</div>
      <div id="wizChoices" class="mt"></div>
      <div id="wizErr" class="wiz-err small mt"></div>
    `;
    const box = body.querySelector("#wizChoices");

    const renderChoice = (choice) => {
      const cid = String(choice?.id || slug(choice?.name || "choice"));
      const need = clampInt(choice?.choose ?? 1, 1, 20);
      const key = `class:${which}:${cls}:L${lv}:choice:${cid}`;
      const opts = Array.isArray(choice?.options) ? choice.options : [];
      const selected = Array.isArray(LEVELUP_WIZ.classChoices?.[key]) ? LEVELUP_WIZ.classChoices[key] : [];

      const optHtml = opts.map(o => {
        const oid = String(o?.id||"");
        const on = String(o?.name||oid);
        const checked = selected.includes(oid) ? "checked" : "";
        return `<label class="pill" style="display:block; margin:6px 0;"><input type="checkbox" data-ckey="${escapeAttr(key)}" data-need="${need}" value="${escapeAttr(oid)}" ${checked}> ${escapeHtml(on)}</label>`;
      }).join("");

      return `
        <div class="card mt" style="padding:10px;">
          <div><strong>${escapeHtml(String(choice?.name||cid))}</strong></div>
          ${choice?.prompt ? `<div class="small muted mt">${escapeHtml(String(choice.prompt))}</div>` : ""}
          <div class="small muted mt">Choose ${need}.</div>
          <div class="mt">${optHtml}</div>
          <div class="small muted mt">Selected: <span data-cnt="${escapeAttr(key)}">${selected.length}</span>/${need}</div>
        </div>
      `;
    };

    if (box) box.innerHTML = choices.map(renderChoice).join("");

    qa("input[type=checkbox][data-ckey]", body).forEach(cb => {
      cb.addEventListener("change", () => {
        const key = cb.dataset.ckey;
        const need = clampInt(cb.dataset.need, 1, 20);
        const cur = Array.isArray(LEVELUP_WIZ.classChoices?.[key]) ? LEVELUP_WIZ.classChoices[key].slice() : [];
        const id = cb.value;
        if (cb.checked){
        ...
        // Enforce exact choose count by trimming extras (keeps earliest checked).
        while (cur.length > need) cur.pop();
        LEVELUP_WIZ.classChoices[key] = cur;

        // Update UI selection state if we trimmed
        const set = new Set(cur);
        qa(`input[type=checkbox][data-ckey]`, body).filter(x => x.dataset.ckey === key).forEach(x => {
          x.checked = set.has(x.value);
        });
        const cnt = qa('span[data-cnt]', body).find(x => x.dataset.cnt === key);
        if (cnt) cnt.textContent = String(cur.length);
      });
    });
  ...
  const hadTough = hasFeatId(state, "feat_tough");
  const curMax = clampInt(Number(state.combat?.hpMax || 0), 0, 99999);

  const rollVal = clampInt(Number(LEVELUP_WIZ.hpRoll || 0), 0, dieMax);
  const baseGainVal = rollVal ? Math.max(1, rollVal + con) : 0;
  LEVELUP_WIZ.hpRoll = rollVal || null;
  LEVELUP_WIZ.baseHpGain = rollVal ? baseGainVal : null;
  // hpGain is finalized in commit (so feat choices like Tough can be applied correctly).
  LEVELUP_WIZ.hpGain = LEVELUP_WIZ.baseHpGain;

  const gainText = (LEVELUP_WIZ.baseHpGain != null) ? String(LEVELUP_WIZ.baseHpGain) : "N/A";
  const toughHint = hadTough ? " +2 (Tough)" : "";
  const newMaxText = (LEVELUP_WIZ.baseHpGain != null) ? String(curMax + LEVELUP_WIZ.baseHpGain + (hadTough ? 2 : 0)) : "N/A";
  ...
  body.innerHTML = `
    <div class="small muted">
      ...
      ${hadTough ? `You have <b>Tough</b>, add <b>+2</b> HP for this level.` : ""}
    </div>
    <div class="row mt">
      <label>Roll (1-${dieMax})
    ...
    <div class="small muted mt">
      ...
      Base HP gain: <b id="wizHpGain">${gainText}</b>${toughHint},
      preview new Max HP: <b id="wizHpNewMax">${newMaxText}</b>.
    </div>
  `;
  ...
  const applyRoll = (v) => {
    ...
    LEVELUP_WIZ.baseHpGain = g;
    LEVELUP_WIZ.hpGain = g;
    if (out) out.textContent = String(g);
    if (outNew) outNew.textContent = String(curMax + g + (hadTough ? 2 : 0));
  ...
  if (stepId === "summary"){
    ...
    const baseHp = LEVELUP_WIZ.baseHpGain ?? LEVELUP_WIZ.hpGain;
    const gainingToughNow = (LEVELUP_WIZ.asiFeat?.type === "feat") && (String(LEVELUP_WIZ.asiFeat?.featId||"") === "feat_tough");
    const toughBonus = toughBonusOnLevelUp({ stateBefore: state, gainingToughNow, newTotalLevel: totalLevelOf(state) + 1 });
    const hp = (baseHp != null) ? (Number(baseHp||0) + Number(toughBonus||0)) : null;
    ...
    body.innerHTML = `
      ...
      <div class="mt">
        <div class="kv"><div>Class</div><div><strong>${escapeHtml(cn)}</strong></div></div>
        <div class="kv"><div>Class level</div><div>${from} → <strong>${to}</strong></div></div>
        <div class="kv"><div>HP gain</div><div><strong>${hp||0}</strong>${toughBonus?` <span class="small muted">(includes Tough +${toughBonus})</span>`:""}</div></div>
        ${(LEVELUP_WIZ.subclassName) ? `<div class="kv"><div>Subclass</div><div><strong>${escapeHtml(LEVELUP_WIZ.subclassName)}</strong></div></div>` : ""}
        ${(LEVELUP_WIZ.steps.includes("choices")) ? `<div class="kv"><div>Class choices</div><div><strong>${Object.keys(LEVELUP_WIZ.classChoices||{}).length}</strong></div></div>` : ""}
...
function wizardValidateAndAdvance(dir){
  ...
  if (stepId === "choose"){
    ...
    // Warlock-only: no secondary class.
    ...
    // Build step list
    const steps = ["choose"];
    // If the class has subclasses and the character hasn't picked one yet, prompt now.
    const hasSubclassOptions = (DATA.subclasses?.classes||[]).some(c => c.class === className && Array.isArray(c.subclasses) && c.subclasses.length);
    if (hasSubclassOptions && !subclassName) steps.push("subclass");
    // Class feature choices at the new class level
    steps.push("choices");
    steps.push("hp");
    ...
    LEVELUP_WIZ.classChoices = {}; // { [choiceKey]: string[] }
  ...
  if (stepId === "subclass"){
    ...
    const sn = String(LEVELUP_WIZ.subclassName||"").trim();
    if (!sn){ wizardSetError("Choose a subclass."); return; }
    LEVELUP_WIZ.step++;
    renderLevelUpWizardStep();
    return;
  }
  ...
  if (stepId === "choices"){
    // Validate any per-level class feature choices introduced at the new class level.
    const cls = (LEVELUP_WIZ.className||"").trim();
    const lv = clampInt(LEVELUP_WIZ.toLevel, 1, 20);
    const which = LEVELUP_WIZ.which;
    const cd = DATA.classFeatures?.classes?.[cls] || null;
    ...
    for (const ch of choices){
      const cid = String(ch?.id || slug(ch?.name || "choice"));
      const need = clampInt(ch?.choose ?? 1, 1, 20);
      const key = `class:${which}:${cls}:L${lv}:choice:${cid}`;
      const picked = Array.isArray(LEVELUP_WIZ.classChoices?.[key]) ? LEVELUP_WIZ.classChoices[key] : [];
      if (picked.length !== need){
        wizardSetError(`Complete: ${String(ch?.name||cid)} (choose ${need}).`);
        return;
      }
    }
  ...
  if (stepId === "hp"){
    ...
    const con = abilityMod(state.abilities?.CON ?? 10);
    LEVELUP_WIZ.baseHpGain = Math.max(1, roll + con);
    LEVELUP_WIZ.hpGain = LEVELUP_WIZ.baseHpGain;
    LEVELUP_WIZ.step++;
    renderLevelUpWizardStep();
    return;
...
function commitLevelUpWizard(){
  ...
  // Subclass (level 1 feature pick, if applicable)
  if (LEVELUP_WIZ.subclassName && !String(block.subclass||"").trim()){
    block.subclass = String(LEVELUP_WIZ.subclassName||"").trim();
  }

  // Apply class choice selections for the new level
  if (LEVELUP_WIZ.classChoices && typeof LEVELUP_WIZ.classChoices === "object"){
    state.classChoices = (state.classChoices && typeof state.classChoices === "object") ? state.classChoices : {};
    for (const [k,v] of Object.entries(LEVELUP_WIZ.classChoices)){
      if (!Array.isArray(v)) continue;
      state.classChoices[String(k)] = Array.from(new Set(v.map(x=>String(x)).filter(Boolean)));
    }
  }
...
function computeSpellModForBlock(block){
  ...
  const ab = SPELL_ABILITY_BY_CLASS[cn] || null;
...
async function boot(){
  ...
  state = loadCharacterState();
  // Warlock-only: hard lock class and remove multiclassing.
  state.multiclass = false;
  state.primary = state.primary || {};
  state.primary.className = "Warlock";
  state.secondary = state.secondary || {};
  state.secondary.className = "";
  state.secondary.classLevel = 0;
  ...
  // Warlock-only: class is always Warlock.
  const primaryClassSel = $("primaryClass");
  if (primaryClassSel){
    primaryClassSel.value = "Warlock";
    primaryClassSel.disabled = true;
  }
  ...
  // Warlock-only: multiclass / secondary UI removed.
